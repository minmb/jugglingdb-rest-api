...
exports.initialize = function(schema, callback) {
    ...
    var endpoint = url.parse(settings.endpoint);
    options.basePath = endpoint.pathname;
    delete endpoint.pathname;
    options.url = url.format(endpoint);
...
RestAPI.prototype._url = function(model, id_or_query) {
    ...
    var url = (this.basePath || '') + '/' + resource;

    if (typeof id_or_query === 'object' && id_or_query !== null) {
        url += '?query=' + encodeURIComponent(JSON.stringify(id_or_query));
    } else if (id_or_query) {
        url += '/' + id_or_query;
    }
